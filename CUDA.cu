...
int main(int argc, char **argv)
{
    ...
    cout<<"Num "<<num1<<" In rank "<<myrank<< endl;
    cout<< "Offset " << offset1 <<" In rank "<<myrank<<endl;
    
    for(int kk=offset1; kk<offset1+num1; kk++){
        
        
        string strZ =" movement " + to_string(kk); 
    ...
    for(auto itr=mvmnt1.begin();itr!=mvmnt1.end();itr++){
      op <<itr->first<<"  "<<itr->second.first<<" "<<itr->second.second<<endl;
